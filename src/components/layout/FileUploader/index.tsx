...
import { getWrapperProps, InputWrapperProps } from 'components/layout/InputWrapper';
...
const FileUploader = (props: FileUploaderProps) => {
  const { value, setFile, isLoading, allowedFiles = ['application/pdf'] } = props;
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!allowedFiles.includes(e.target.files[0].type)) {
      return Toasts.notifyCommon([i18n.t('notifications:error.notAllowedFileFormat')]);
    }
  ...
  return (
    <InputWrapper {...wrapperProps}>
      <div className={style.container}>
        {!value && (
          <Button
            label={i18n.t('common:action.chooseFile')}
            onClick={handleLoad}
            isLoading={isLoading}
